...
class TaskProvider with ChangeNotifier {
  ...
  Future<void> addtasktoList(String titleofTask, String commentofTask,
    ...
    final urlImage =
        'http://192.168.0.111:5000/upload/${basename(imageFile.path)}/${newObject.id}';
    try {
      ...
      final response1 = await http.put(urlImage, body: await imageFile.readAsBytes());
  ...
  Future<void> deleteItem(String idItem, String username) async {
    final existingItemindex =
        _list.indexWhere((element) => element.id == idItem);
    var existingItem = _list[existingItemindex];
    _list.remove(existingItem);
    notifyListeners();
    final url = 'http://192.168.0.111:5000/deleteItem/$username/$idItem';
    final response =
